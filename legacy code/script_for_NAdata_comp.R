### doing the same wihtout correcting GPA
...
library(sandwich)
library(pbkrtest)
data.file.100000 <- read.sav("BARCS_Data_used_for_paper.sav")
data.sem1 <- data.file.100000 %>% filter(Semester == 1)
data.sem2 <- data.file.100000 %>% filter(Semester == 2)
data.sem3 <- data.file.100000 %>% filter(Semester == 3)
data.sem4 <- data.file.100000 %>% filter(Semester == 4)
...
data.file.100000.long <- rbind(data.SEM1, data.SEM2, data.SEM3, data.SEM4)

data.file.100000.long <- data.file.100000.long %>%
...
data.file.100000.long <- data.file.100000.long %>% mutate(Cluster_current = as.factor(Cluster_current),
                                            BARCS_ID = as.factor(BARCS_ID))


## Data selection required to identify if GPA = 0 and SAT = 0 are NAs
ind.gpa0 <- data.file.100000.long %>% filter(GPA == 0) %>% select(BARCS_ID, Semester, GPA)
ind.gpa0.vector <- unique(ind.gpa0$BARCS_ID)
ind.gpa0 <- data.file.100000.long %>% filter(BARCS_ID %in% ind.gpa0.vector) %>% group_by(BARCS_ID, Semester)
ind.sat0 <- data.file.100000.long %>% ungroup() %>% filter(SATTotal == 0) %>% select(BARCS_ID, Semester, SATTotal, GPA)


# ### imputing NA for false classification of SAT / GPA = 0 
# data.file.100000.long <- data.file.100000.long %>% mutate(GPA = replace(GPA, GPA == 0, NA),
...
data.file.100000.long <- data.file.100000.long %>% group_by(BARCS_ID) %>% 
  mutate(sum.GPAna = sum(is.na(GPA)))

### include average GPA for each student 
data.file.100000.long <- data.file.100000.long %>% group_by(BARCS_ID) %>% mutate(average_GPA = mean(GPA))
#data.file.100000.long <- data.file.100000.long %>% arrange(sum.GPAna, average_GPA)
...
subset.0nagpas <- data.file.100000.long %>% filter(sum.GPAna == 0) %>% arrange(average_GPA)
subset.1nagpas <- data.file.100000.long %>% filter(sum.GPAna == 1) %>% arrange(average_GPA)
subset.2nagpas <- data.file.100000.long %>% filter(sum.GPAna == 2) %>% arrange(average_GPA)
subset.3nagpas <- data.file.100000.long %>% filter(sum.GPAna == 3) %>% arrange(average_GPA)
...
data.file.100000.long <- data.file.100000.long %>% mutate( Time = Semester,
